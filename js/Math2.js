...
for(var i=0;i<names.length; i++){window[names[i]] = Math[names[i]];}
window["Random"] = function(){return Math.random()}
window["randomf"] = function(max, min){return Math.random()*(max-min)+min;}
window["random"] = function(max, min){return Math.round(Math.random()*(max-min)+min)}
var seg = new Date().getTime();
